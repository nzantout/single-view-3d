...
def get_args_parser():
    ...
    parser.add_argument('--checkpoint_suffix', default='', type=str) 
...
def save_plot(thresholds, avg_f1_score, args):
    ...
    plt.savefig(f'eval_{args.type}{args.checkpoint_suffix}.png', bbox_inches='tight', format='png')
...
def evaluate_model(args):
    ...
    if args.load_checkpoint:
        checkpoint = torch.load(f'checkpoint_{args.type}{args.checkpoint_suffix}.pth')
        model.load_state_dict(checkpoint['model_state_dict'])
        print(f"Succesfully loaded iter {start_iter}")
    
    print("Starting evaluating !")
    max_iter = len(eval_loader) if args.end_iter == -1 else args.end_iter
    for step in range(start_iter, max_iter):
        ...
        if (step % args.vis_freq) == 0:
            # visualization block
            if args.type == 'vox':
                rendering.render_voxels(
                    predictions, 
                    f'vis/{step}_{args.type}{args.checkpoint_suffix}_pred.gif',
                    thresh=0.2,
                    dist=3,
                    azim_start=0,
                    azim_end=360,
                    light_location=[[0, 0, 3]])
                pred_confs_mesh = rendering.cubify_voxels_confidence(predictions[0, 0, ...])
                rendering.render_mesh(
                    pred_confs_mesh, 
                    f'vis/{step}_{args.type}{args.checkpoint_suffix}_confs.gif',
                    dist=1.5,
                    azim_start=0,
                    azim_end=360,
                    light_location=[[0, 0, 3]],
                ...
                rendering.render_pointcloud(predictions, f'vis/{step}_{args.type}{args.checkpoint_suffix}_pred.gif', dist=1.5)
            else:
                rendering.render_mesh(predictions, f'vis/{step}_{args.type}{args.checkpoint_suffix}_pred.gif', dist=1.5)

            rendering.render_mesh(mesh_gt.to(args.device), f'vis/{step}_{args.type}{args.checkpoint_suffix}_gt.gif', dist=1.5)
            
            plt.imsave(f'vis/{step}_{args.type}{args.checkpoint_suffix}.png', images_gt[0].cpu().detach().numpy())
